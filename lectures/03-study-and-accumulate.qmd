...
| | |
|---|---|
| 1 | **domain** — where does this mean anything? |
| 2 | **zeros** — where does it cross? |
| 3 | **sign** — where is it positive, where negative? |
| 4 | **stationary points** — where is `f'(q) = 0`? |
| 5 | **nature** — maximum or minimum, from the sign of `f''` |
| 6 | **shape** — concave or convex, any inflection |
| 7 | **sketch** — last, not first |

[Do them in this order every time. The sketch is the *result* of the study, not a substitute for it.]{.red}

## Seven steps, watched once {.clip background-color="#000000"}

<video class="clipvideo" width="1000" height="562" controls preload="metadata" poster="media/w03_study_poster.png"><source src="media/w03_study.mp4" type="video/mp4"></video>
...
2. **Zeros** `q² − 300q + 15,000 = 0` → `63.4`, `236.6`.
3. **Sign** positive between the roots.
4. **Stationary** `π'(q) = −8q + 1200 = 0` → `q = 150`.
:::
::: {.column width="50%"}
5. **Nature** `π''(q) = −8 < 0` → a maximum, `π(150) = 30,000`.
6. **Shape** concave everywhere: no inflection.
7. **Sketch** with all four key points labelled.
:::
::::
...
## Marginal cost is per bike. Total cost is for all of them.

If the tenth bike costs €11 and the eleventh €11.80, what do the first twenty cost together?

::: {.callout-important title="Bet before we compute"}
Guess the total variable cost of the first twenty bikes when `MC(q) = 0.8q + 3`.
:::

Add up the cost of each one. Thin the slices until the answer stops moving.

## Thinner slices, truer area {.clip background-color="#000000"}

<video class="clipvideo" width="1000" height="562" controls preload="metadata" poster="media/w03_riemann_poster.png"><source src="media/w03_riemann.mp4" type="video/mp4"></video>

180, then 200, then 212, then 217 — climbing to 220.

## Two facts, and you can do every exam integral
...
Now every term is a power or a log.
:::
::::
