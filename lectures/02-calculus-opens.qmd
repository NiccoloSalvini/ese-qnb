...
$$\sqrt{q-20} \;\text{ needs } q \geq 20 \qquad \ln(q-5) \;\text{ needs } q > 5 \qquad \frac{120}{q-40} \;\text{ breaks at } q = 40$$
...
Never ask "what is the derivative of this?" Ask the four questions in order.

| | ask | then |
|---|---|---|
| **1** | Is it a **sum**? | split it, and do each piece on its own. The rest of this table is for one piece. |
| **2** | Is it a **number** with no `q`? | it becomes **0**. `−60,000` disappears. |
| **3** | Is it a **power** `a·qⁿ`? | **bring `n` down in front, then drop it by one**: `−4q² → −8q`. (A lone `q` is `q¹`, so it becomes the number in front of it: `1200q → 1200`.) |
| **4** | Is it a **log** `a·ln(q)`? | it becomes `a/q`. And `ln(q+1)` becomes `1/(q+1)`. |

. . .

$$\pi(q) = \underbrace{-4q^{2}}_{\text{rule 3}} + \underbrace{1200q}_{\text{rule 3}} \underbrace{- \,60{,}000}_{\text{rule 2}} \;\longrightarrow\; \pi'(q) = -8q + 1200$$

[Four questions, every time, in that order. That is the whole of what the midterm's study of a function needs you to do.]{.red}
