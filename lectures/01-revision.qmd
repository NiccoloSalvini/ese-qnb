...
[Every technique in this room is here because an assessment asks for it. I will tell you which one, every time.]{.red}
...
The founder learns: at €1,000 she sells 150 bikes. Every €4 off the price sells one more bike; every €4 on it sells one fewer, so at **€1,600 she sells none**. That is the line:
...
## Try it: move a, b and c

<iframe class="app" src="apps/function-lab.html?f=parab&a=-1&pb=2&c=3" loading="lazy"></iframe>

[Make $a$ positive, then negative. Slide $c$ and watch the whole curve move up and down. Find a setting with no roots: what happens to the profitable window?]{.muted style="font-size:.7em"}
...
![](img/w01_tolerance.svg){width="64%" fig-align="center"}

Cells #1 (17 Wh short) and #4 (16 Wh over) go back.

## What |·| does to a graph

![](img/w01_abs_flip.svg){.nostretch fig-align="center" width="88%"}

Everything **below** the axis is flipped **up**, like a mirror. Everything above stays where it is. So an absolute value is **never negative**, and its graph has a corner where the original crossed zero.

## Solve by looking

![](img/w01_abs_solve.svg){.nostretch fig-align="center" width="82%"}

:::: {.columns}
::: {.column width="34%"}
**$\le$** — below the level: **one interval**, $-1 \le x \le 5$.
:::
::: {.column width="33%"}
**$>$** — above the level: **two tails**, joined by "or", never "and".
:::
::: {.column width="33%"}
**Level below zero** — a V never goes under the axis: no solution. With $>$ it would be *every* $x$.
:::
::::

## Try it: flip it, cut it, solve it {.lab-app}

<iframe class="app" src="apps/function-lab.html?f=line&m=1&b=-2&op=abs&k=3&rel=le" loading="lazy"></iframe>

[1. Switch to a parabola and flip $x^{2} - 4$. &nbsp; 2. Press $\sqrt{\;}$: where does the graph vanish, and why? &nbsp; 3. Parabola with $a = 1$, $b = 0$, $c = 0$, then $\sqrt{\;}$: you get $|x|$, because $\sqrt{x^{2}} = |x|$.]{.muted style="font-size:.7em"}

## One trap the picture will not show you

$$|a + b| \ne |a| + |b| \qquad\qquad |3 + (-5)| = 2, \;\text{ not } 8$$

It **does** split over a product: $|a\,b| = |a|\,|b|$.

[When the exam asks for an interval: $[\,]$ includes the endpoint, $(\,)$ excludes it and always goes with $\pm\infty$.]{.muted}
