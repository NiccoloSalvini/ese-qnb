...
## The absolute value: the rules, and where marks are lost
...
It removes the sign and keeps the size: $|{-7}| = |7| = 7$.

**Solving with it**

- $|x - a| \le b \iff a - b \le x \le a + b$: one interval, **and**
- $|x - a| > b \iff x < a - b$ **or** $x > a + b$: two tails, never "and"
- $|x - a| = b$ has **two** answers, $a \pm b$
:::
::: {.column width="50%"}
**The traps**

- **The right-hand side can be negative.** $|x| \le -3$ has no solution; $|x| > -3$ is true for every $x$. Check its sign before anything else.
- **It does not split over a sum.** $|a + b| \ne |a| + |b|$ in general: $|3 + (-5)| = 2$, not $8$. It does split over a product: $|ab| = |a|\,|b|$.
- $\sqrt{x^{2}} = |x|$, not $x$: $\sqrt{(-4)^{2}} = 4$.
- **Brackets.** $[\,]$ includes the endpoint, $(\,)$ excludes it and always goes with $\pm\infty$. The exam wants intervals written this way.
:::
::::
